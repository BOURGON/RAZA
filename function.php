...
class Inscription {
	...
	public static function inscrire($nom, $prenom, $identifiant, $email, $passeUn, $passeDe) {
		...
		if(!empty($nom) AND !empty($prenom) AND !empty($identifiant) AND !empty($email) AND !empty($passeUn) AND !empty($passeDe)) {
			...
			$resultat = '<span class="error-info">Vous devez remplir tous les champs, <br />veuillez recommencer l\'inscription.</span>';
			echo' <form action="index.php" method="post">
							<input type="submit" value="Retour à l\' espace personnel" />
	...
	public static function activer($pseudo) {
		...
		switch($methode['id']) {
			...
			$resultat = 'Votre cr&eacute;ation de compte est termin&eacute;e, vous pouvez maintenant vous connecter.<br /><a href="'.URLSITE.'/index.php"><span class="blue">Connexion</span></a>';
			...
			$resultat = 'Votre cr&eacute;ation de compte est termin&eacute;e, un email de confirmation vient de vous &ecirc;tre envoy&eacute;,<br />pensez a v&eacute;rifier vos spams.';
			  echo'<form action="index.php" method="post">
...
################################################test juillet 2014 ###########################################
	...
	public static function newPass2($id, $newPassUn, $newPassDe) {
		...
		if(!empty($newPassUn) AND !empty($newPassDe)) {
			...
			if($newPassUn === $newPassDe) {
				...
				$dataPass = $verifPass -> fetch(PDO::FETCH_ASSOC);
					...
					if($majPass -> execute()) {
						...
						$resultat = '<div class="alert alert-success">Votre mot de passe a &eacute;t&eacute; chang&eacute; avec succ&egrave;s.</div>';
						...
						$resultat = '<div class="alert alert-warning"><span class="error-info">Une erreur est survenue pendant la mise &agrave; jour de votre mot de passe.</span></div>';
				...
				$resultat = '<div class="alert alert-warning"><span class="error-info">Les champs &quot;Votre nouveau mot de passe&quot; et &quot;Saisir &agrave; nouveau le mot de passe&quot; doivent &ecirc;tre identiques,<br />veuillez recommencer.</span></div>';
			...
			$resultat = '<div class="alert alert-warning"><span class="error-info">Pour changer de mot de passe vous devez remplir tout les champs,<br />veuillez recommencer.</span></div>';
		...
		while($donnee = $resultat -> fetch(PDO::FETCH_ASSOC)) {
			...
			elseif($donnee['activation'] === '0') {
				...
				switch($donnee['niveau']) {
					...
					$niveau = '<button type="button" class="btn btn-warning">Mod&eacute;rateur</button>';
					...
					$niveau = '<button type="button" class="btn btn-success"> Administrateur </button>';
...
<div class="col3"><center>'.$niveau.'</center></div>
